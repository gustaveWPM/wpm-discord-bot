...
const [fromTable, toTable] = [prisma.tempBannedMembersMissingPermsToUnban, prisma.tempBannedMembers] as const;
...
async function tryToProcessTransaction({
  ...
  const __firstFilter = tempBannedMembersMissingPermsToUnban.filter((newMemberCandidate) =>
  ...
  if (!botMemberIsAuthorizedToManageBans) {
    traceError(new Error(skillIssueMsg), { from: attemptToMoveTempBannedMembersMissingPermsToTempBannedMembers.name });
  }
...
async function attemptToMoveTempBannedMembersMissingPermsToTempBannedMembers(guild: Guild) {
  const { id: guildId } = guild;
      ...
      .run(async () => {
        try {
          ...
          if (
            ...
            traceError(error, {
              from: attemptToMoveTempBannedMembersMissingPermsToTempBannedMembers.name,
              ctx: { guildId }
            });
          }
        }
...
export default attemptToMoveTempBannedMembersMissingPermsToTempBannedMembers;
