import type { TempBannedMembersMissingPermsToUnban } from '@prisma/client';
...
const [fromTable, toTable] = [prisma.tempBannedMembersMissingPermsToUnban, prisma.tempBannedMembersAbandoners] as const;
...
async function tryToProcessTransaction({
  tempBannedMembersMissingPermsToUnban,
  guildIdAsBigInt,
  guildId
}: {
  ...
  const filteredTempBannedMembersMissingPermsToUnban = tempBannedMembersMissingPermsToUnban.filter((newMemberCandidate) =>
    ...
    cb: async () =>
      await prisma.$transaction(
        [
          toTable.deleteMany({
            where: {
              discordUserId: { in: filteredTempBannedMembersMissingPermsToUnban.map((member) => member.discordUserId) },
              discordGuildId: guildIdAsBigInt
            }
          }),

          toTable.createMany({
            data: filteredTempBannedMembersMissingPermsToUnban.map((member) => ({
...
async function attemptToMoveTempBannedMembersMissingPermsToTempBannedMembersAbandoners(guildId: Guild['id']) {
  try {
    await verrou
      .use('redis')
      ...
      .run(async () => {
        try {
          ...
          const maybeTempBannedMembersMissingPermsToUnban: TempBannedMembersMissingPermsToUnban[] = await fromTable.findMany({
            where: { discordGuildId: guildIdAsBigInt }
          });

          if (maybeTempBannedMembersMissingPermsToUnban.length > 0) {
            await tryToProcessTransaction({
              tempBannedMembersMissingPermsToUnban: maybeTempBannedMembersMissingPermsToUnban,
              guildIdAsBigInt,
              guildId
            });
          }
          ...
          if (
            ...
            traceError(error, {
              from: attemptToMoveTempBannedMembersMissingPermsToTempBannedMembersAbandoners.name,
              args: { guildId }
            });
          }
        }
...
export default attemptToMoveTempBannedMembersMissingPermsToTempBannedMembersAbandoners;
