...
const [fromTable, toTable] = [prisma.tempBannedMembers, prisma.tempBannedMembersMissingPermsToUnban] as const;
...
async function attemptToMoveTempBannedMembersToTempBannedMembersMissingPerms(guildId: Guild['id']) {
  try {
    await verrou
      .use('redis')
      ...
      .run(async () => {
        try {
          ...
          if (
            ...
            traceError(error, {
              from: attemptToMoveTempBannedMembersToTempBannedMembersMissingPerms.name,
              args: { guildId }
            });
          }
        }
...
export default attemptToMoveTempBannedMembersToTempBannedMembersMissingPerms;
